...
class Posseg
{
    ...
    public static function init(Jieba $jieba)
    {
        /** @var Jieba jieba */
        self::$jieba = $jieba;
        ...
        if (self::$jieba::$dictname != '') {
            $content = JiebaCo::getFileContent(self::$jieba::$dictname,'txt');
        ...
        if (sizeof(self::$jieba::$user_dictname) != 0) {
            for ($i = 0; $i < sizeof(self::$jieba::$user_dictname); $i++) {
                $content = JiebaCo::getFileContent(self::$jieba::$user_dictname,'txt');
                foreach ($content as $line) {
                    $word = $line[0];
                    $freq = $line[1];
                    $tag = $line[2];
        ...
        $content = JiebaCo::getFileContent(dirname(__DIR__) . '/dict/pos_tag_readable.txt','txt');
        foreach ($content as $line) {
            $tag = $line[0];
            $meaning = $line[1];
    ...
    public static function loadModel($f_name)
    {
     ...
     * @return array
     */
    public static function getTopStates($t_state_v, $top_k = 4)
    {
        arsort($t_state_v);
     ...
     * @return array
     */
    public static function viterbi($sentence)
    {
        $obs = $sentence;
     ...
     * @return array
     */
    private static function __cut($sentence, $options = array('HMM' => true))
    {
        $words = array();
     ...
     * @return array
     */
    public static function __cutDetail($sentence)
    {
        $words = array();
     ...
     * @return array
     */
    public static function __cutDAG($sentence)
    {
        $words = array();

        $N = mb_strlen($sentence, 'UTF-8');
        $DAG = self::$jieba::getDAG($sentence);

        self::$jieba::calc($sentence, $DAG);

        $x = 0;
        $buf = '';

        while ($x < $N) {
            $current_route_keys = array_keys(self::$jieba::$route[$x]);
     ...
     * @return array
     */
    public static function __cutDAGNoHMM($sentence)
    {
        $words = array();
     ...
     * @return array
     */
    public static function posTagReadable($seg_list)
    {
        $new_seg_list = array();
